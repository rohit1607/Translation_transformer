...
# from src.mae_all_data_load import get_path_names, GiveMe_loaders
...
# from src.custom_models import TAESD
...
class TAESD(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x.squeeze(dim=0) #TODO remove hardcode for perlin
        # image_raw = self.scale_input(x.to(self.dev)) 
        x = x.to(self.dev)
        image_enc = self.encoder(x).to(self.dev)
...
def unnormalize(channel, stats, eps=1e-5):
    cmin, cmax = stats
    channel = channel*(cmax - cmin + eps) +  cmin
    return channel

def unscale_sample(im, stats):
    stats_c0, stats_c1, stats_c2 = stats
    im[0,0] = unnormalize(im[0,0], stats_c0)
    im[0,1] = unnormalize(im[0,1], stats_c1)
    im[0,2] = unnormalize(im[0,2], stats_c2)
    return im


def test_reconstruction_plot_perlin(sample, tae_model, device='cuda', fname="tae_test_sample_pred", path="", eps = 1e-8):
    tae_model.eval()
    with torch.no_grad():
        ...
        output_image = tae_model(image.unsqueeze(0).to(device)).cpu().detach().numpy()
        image = image.detach().cpu().numpy()
        output_image = unscale_sample(output_image, stats)
        image = unscale_sample(image, stats)

        plt.clf()
        fig, axs = plt.subplots(1, 4, figsize=(20,5), gridspec_kw={'width_ratios': [1, 1, 1, 1]})
        ...
        vmag_image = (image[0][0]**2 + image[0][1]**2)**0.5 # TODO remove hardcode for perlin image[0][0]
        ...
        ax = axs[0]
        original_im = plot_vel_field(ax, vmin, vmax, image[0][0], image[0][1], image[0][2], title_name=f"Original") # TODO remove hardcode for perlin image[0][0] vs image[0] for dg3
        divider = make_axes_locatable(ax)
        cax_vel = divider.append_axes("right", size="5%", pad=0.1)
        cbar = fig.colorbar(original_im, cax=cax_vel) #, ticks=np.linspace(vmin, vmax, 8)
        ...
        vmag_reconstructed = (output_image[0][0]**2 + output_image[0][1]**2)**0.5 
        reconstructed_im = plot_vel_field(ax, vmin, vmax, output_image[0][0], output_image[0][1], vmag_reconstructed, title_name=f"Reconstructed")
        divider = make_axes_locatable(ax)
        cax_vel = divider.append_axes("right", size="5%", pad=0.1)
        ...
        ax = axs[2]
        vx_difference = np.abs(image[0][0] - output_image[0][0])
        vy_difference = np.abs(image[0][1] - output_image[0][1])
        # abs_difference = np.abs(image[0] - output_image[0])
        vmag_difference = (vx_difference**2 + vy_difference**2)**0.5 
        vmin = np.min(vmag_difference)
        vmax = np.max(vmag_difference)
        vmin = 0.
        vmax = 1.
        abs_error_1 = np.abs(vmag_image- vmag_reconstructed)
        mean_diff_of_vec_norm= np.mean(abs_error_1)
        rel_error_1 = abs_error_1/ (vmag_image + eps)
        zeros_1 = np.where(vmag_image == 0)
        
        difference_image = plot_vel_field(ax, vmin, vmax, vx_difference, vy_difference, np.abs(image[0][2] - vmag_reconstructed), 
                                          title_name=f"abs_error; {mean_diff_of_vec_norm=}",
                                        #   conturf_mat=np.abs(vmag_image- vmag_reconstructed)/ (vmag_image + eps)
                                        conturf_mat=np.abs(vmag_image- vmag_reconstructed)

                                          ) # TODO remove hardcode for perlin image[0][0]
        divider = make_axes_locatable(ax)
        ...
        mean_norm_of_vec_diff = np.mean(vmag_difference) 

        ax = axs[3]
        difference_image = plot_vel_field(ax, vmin, vmax, vx_difference, vy_difference, vmag_difference, 
                                          title_name=f"abs_error; {mean_norm_of_vec_diff=}",
                                        #   conturf_mat=vmag_difference / (vmag_image + eps),
                                          conturf_mat=vmag_difference 
                                          )
        divider = make_axes_locatable(ax)
        cax_dif = divider.append_axes("right", size="5%", pad=0.1)
        cbar = fig.colorbar(difference_image, cax=cax_dif) #, ticks=np.linspace(vmin, vmax, 8)
...
def main():
    ...
    datapaths = get_path_names(cfg.Train_data_path, dataset='DG3_multi_obs')
    # train_dataloader, val_dataloader, data_sets, test_dat = GiveMe_loaders(*datapaths, 
    #                                    batch_size=cfg.batch_size, path=cfg.tae_path, plot_dat=cfg.plot_dat, cfg=cfg, dataset=cfg.dataset, obstacle_mask=cfg.obstacle_mask)    
    ...
    with torch.no_grad():
        vel_field_data = load_vel_DG3(cfg.aftrain_testDatpath, config=cfg, obstacle_mask='data', dataset='DG3_multi_obs')
        # vel_field_data = load_vel_perlin(cfg.aftrain_testDatpath, cfg, obstacle_mask='mag')
        dataset = VelocityDataset(vel_field_data,  config=cfg) # obstacle_mask='mag'
        i = 0
        sample = dataset[i]
        
        # test_reconstruction_plot_perlin(image, bestModel, fname="Translation_transformer/my-translat-transformer/tmp/debug_tae/perlin_results_6.png", path=model_save_dir)
        test_reconstruction_plot_perlin(sample, bestModel, fname="Translation_transformer/my-translat-transformer/tmp/debug_tae/dg3_multi_obs_results_1.png", path=model_save_dir)
