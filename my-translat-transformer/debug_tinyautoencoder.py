...
from src.mae_all_data_load import plot_vel_field, GiveMe_loaders, load_vel_DG3, VelocityDataset
...
def train_reconstruction_plot(dataset, tae, i, sth=0, fname="tae_validation_(", path="", ep=99, st_eps=0):
    ...
    tname = fname+str(i)+")dataset_("+str(sth)+")original"
    tname2 = fname+str(i)+")dataset_("+str(sth)+")reconstructed"
    tname3 = fname+str(i)+")dataset_("+str(sth)+")abs_difference"
    tname4 = fname+str(i)+")dataset_("+str(sth)+")all"   
    tae.eval()
    taesd.to(device)
    with torch.no_grad():
        output_image = (taesd.unscale_output(taesd(image.unsqueeze(0).to(device)))).cpu().detach().numpy()
        ...
        vmag_image = (image[0]**2 + image[1]**2)**0.5 # TODO remove hardcode for perlin image[0][0]
        ...
        ax = axs[0]
        original_im = plot_vel_field(ax, vmin, vmax, image[0], image[1], image[2], title_name=f"Original") # TODO remove hardcode for perlin image[0][0] vs image[0] for dg3
        ...
        ax = axs[2]
        vx_difference = np.abs(image[0] - output_image[0][0])
        vy_difference = np.abs(image[1] - output_image[0][1])
        ...
        mean_error = np.mean(np.abs(vmag_image - vmag_reconstructed))
        difference_image = plot_vel_field(ax, vmin, vmax, vx_difference, vy_difference, np.abs(vmag_image - vmag_reconstructed), title_name=f"Difference; {mean_error=}") # TODO remove hardcode for perlin image[0][0]
        divider = make_axes_locatable(ax)
        ...
        vmag_difference_computed = (vx_difference**2 + vy_difference**2)**0.5          
        ax = axs[3]
        difference_image = plot_vel_field(ax, vmin, vmax, vx_difference, vy_difference, vmag_difference_computed, title_name=f"Difference; {mean_error=}")
        divider = make_axes_locatable(ax)
        cax_dif = divider.append_axes("right", size="5%", pad=0.1)
        cbar = fig.colorbar(difference_image, cax=cax_dif) #, ticks=np.linspace(vmin, vmax, 8)
        cbar.set_label('Difference')
        # fig.tight_layout()
        plt.savefig(path+"/"+tname4+".png")
...
def main():
    ...
    with torch.no_grad():
        vel_field_data = load_vel_DG3(cfg.aftrain_testDatpath, cfg)
