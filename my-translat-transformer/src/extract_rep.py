...
class ExtractRep:
    ...
    def extract_latent_rep(self, flow_dir, rzn):
        ...
        with torch.no_grad():
            ...
            print(latent_reps[8][:,0].shape) # Verified using self.mae.vit.embeddings(preprocessed_vx_vy)[0][0,0,0:10] and latent_reps[0][0,0,0:10]
            
        return latent_reps[8][:,0].cpu() #shape (120, rep_dim) 
...
if __name__ == "__main__":
    ...
    for idx in range(len(traj_dataset)):
        ...
        # break
    save_dir = os.path.join(ROOT, f"Gathered_datasets/gathered_{gather_name}_50")
