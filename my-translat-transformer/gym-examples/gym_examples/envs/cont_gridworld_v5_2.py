""""
cont_gridworld_v5_2 as per updated api: https://www.gymlibrary.ml/content/environment_creation/
# Contnuous states. n Discrete Actions; multiple possible starting points; with velocity field
v5_2:
 - derived from v5_1
 - IMP: Takes in multiple target positions, but considers them one at a time
        based on target_id. So you can switch target positons by resetting the env,
        and potentially avoid reloadin velocities
 - scale_velocity behavior from v5_1
 - does not deal with obstacles (like v5_1)
 - this is for loading perlin volicities (unlike both v5 and v5_1) 
    because perlin flows are 1 rzn per file. So there is no all_ui, all_vi,
"""
...
# Contnuous states. n Discrete Actions; multiple possible starting points; with velocity field
class ContGridWorld_v5_2(gym.Env):
    ...
    def __init__(self, state_dim=2, action_dim=1, action_range=[0.0,2*math.pi], grid_dim=[10.,10.],start_pos=[[5.0,5.0]], target_pos=[[8.0,8.0]], target_rad=1, F=1):
        ...
        print(f"**** initializing ContGridWorld_v5_2 environment *****")
        #default values
    ...
    #  remove later
    def tmp_infer_target_id(self, query_final_pos, eps=0.1):
        for i, tgt_pos in enumerate(self.target_pos):
            if np.linalg.norm(np.array(query_final_pos) - np.array(tgt_pos)) < eps:
                return i
        
        
    def setup(self, cfg, params2, target_id, denorm_final_pos=None, rzn=0, add_trans_noise=False):
        """
        ...
        # NOTE: self.target_pos in v5_2, is one target position from a list, and is indexed by
        #           target_id. If target_id is set to -1, then infer target_id from target_pos list
        self.target_pos = np.array(self.params2['end_pos_us'])*self.space_scale
        if target_id == -1 and denorm_final_pos is not None:
            query_final_pos = denorm_final_pos*self.space_scale
            target_id = self.tmp_infer_target_id(query_final_pos)
            self.target_id = target_id
        self.target_pos = np.array(self.params2['end_pos_us'])[target_id]*self.space_scale
        ...
        # vx_vy = np.float32(np.load(self.vel_fname +'/all_vxvy.npy')).transpose(1,0,2,3)
        vx_vy = np.float32(np.load(self.vel_fname +'/all_vxvy.npy')).transpose(1,0,2,3) # Change to all_vxvy_interpolated for perlin
        self.U = vx_vy[:,0]
        self.V = vx_vy[:,1] # (t,i,j) 
        
        self.dxy = float(self.xlim)/self.U.shape[1]
        self.nT = self.U.shape[0]
        self.n_rzns = 1 #self.Yi.shape[1]
